...
namespace OO___Hospital
{
    class Data
    {
        private string connectionString = "datasource=127.0.0.1;port=8889;username=root;password=root;database=hospital;";
        ...
        public List<Person> SelectStaff(int hospitalID)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    {
                        staff.Add(new Doctor((int)reader["ID"], (string)reader["Name"], DateOnly.FromDateTime((DateTime)reader["Birth"]), (string)reader["Specialty"]));
                    }
                    else
                    {
                        Area area;
